...
#endif
    pde_t *pdep = &pgdir[PDX(la)];//获取页目录项的地址
    if (!(*pdep & PTE_P)) {//如果页目录项记载它指向的二级页表还没有分配物理空间
        ...
        //下面都是给新页初始化属性
        set_page_ref(page, 1);
        uintptr_t pa = page2pa(page);//这个页的物理地址
        memset(KADDR(pa), 0, PGSIZE);
        *pdep = pa | PTE_U | PTE_W | PTE_P;//把页目录项的内容填好，地址是二级页表的物理地址，后面是权限之类的
