...
{     
     mm->sm_priv = NULL;//去掉了哨兵，链表里全是有意义的vam
     return 0;
}
...
{
    //将一个page加入到mm管理的链表中去，元素是已经在内存中的物理页
...
_lru_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     assert(in_tick==0);
     int min_heat = 0;
     list_entry_t *selected , *p = head;
    selected = p;
    min_heat = le2page(p,pra_page_link)->heat;
     do{
            if(le2page(p,pra_page_link)->heat < min_heat)
            {
                selected = p;
                min_heat = le2page(p,pra_page_link)->heat;
            }
